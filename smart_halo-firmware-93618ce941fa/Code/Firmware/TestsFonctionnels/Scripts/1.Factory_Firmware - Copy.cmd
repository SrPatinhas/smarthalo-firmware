...
for /f "usebackq delims=" %%a in ("Hex_file_names.ini") do (
    ...
    if "x!ln:~0,1!"=="x[" (
		...
		set bootloaderVal=%%c
            ...
            if "x!settingsKey!"=="x!currkey!" (          
		set settingsVal=%%c
            )
            if "x!softdeviceKey!"=="x!currkey!" (          
		set softdeviceVal=%%c
            )
        )
    )
...
mergehex --merge !applicationVal! !softdeviceVal! --output merged.hex
mergehex --merge merged.hex !settingsVal! --output merged_settings.hex
mergehex --merge merged_settings.hex !bootloaderVal! --output Boot_s132_firm_merge.hex
