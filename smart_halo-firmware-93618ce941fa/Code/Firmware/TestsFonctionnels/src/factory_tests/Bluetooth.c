...
#define TF_DEVICE_NAME                      "SMARTHALO_EE_UART"                         /**< Name of device. Will be included in the advertising data. */
...
void Bluetooth_onNewData(uint8_t * p_data, uint16_t length)
{
	...
	static uint8_t	LastPacketReceived[BLE_NUS_MAX_DATA_LEN] = {0,0};
	...
	if((memcmp(LastPacketReceived, p_data, length) == 0)&&(length==BLE_NUS_MAX_DATA_LEN))
	{
	...
	{
		memcpy(LastPacketReceived, p_data, (length)*sizeof(uint8_t));
	}
...
void Bluetooth_resetTimerCnt()
{
	...
	ble_tx_char_timer_cnt = app_timer_cnt_get();
}
...
int Bluetooth_sendPacket()
{
	...
	if( (lengthSent >= BLE_NUS_MAX_DATA_LEN) || Bluetooth_TXcharHasTimedOut() )
	{
		...
		// if the same data is sent twice, do nothing
		if((memcmp(LastPacketSent, tempBuff, lengthSent) == 0)&&(lengthSent==BLE_NUS_MAX_DATA_LEN))
		{
			//memcpy(LastPacketSent, p_data, (length)*sizeof(uint8_t));
			//return;
		}
		...
		{
			memcpy(LastPacketSent, tempBuff, (lengthSent)*sizeof(uint8_t));
