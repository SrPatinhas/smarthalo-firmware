...
static bool touchtest_is_running = false;
...
static void TouchTest_heartbeat_timer_handler(void * p_context)
{
	if(heartbeatcount == 0)
	{
		printf("Heartbeat count test completed with error: No heartbeat detected\r\n");
	}
//	else if(heartbeatcount%2)
//	{
//		printf("Heartbeat count test completed with error: %d is an odd edge count\r\n", heartbeatcount);
//	}
	else
	{
		printf("Heartbeat count test completed successfully: %d heartbeats in %d seconds\r\n", heartbeatcount/2, timeout);
	}

	nrf_drv_gpiote_in_uninit(TOUCHPIN);

	touchtest_is_running =false;
