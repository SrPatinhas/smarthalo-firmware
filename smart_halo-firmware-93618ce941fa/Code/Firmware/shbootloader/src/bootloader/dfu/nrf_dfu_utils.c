...
void nrf_dfu_utils_showProgress(float progress) {
    leds_direct_set(0,progress*MAXLUM,0);
}
...
static uint32_t nrf_dfu_app_continue(uint32_t               src_addr)
{
    ...
    uint32_t length_total        = (image_size - s_dfu_settings.write_offset);
    uint32_t cur_len;
    uint32_t crc;

    printf("Enter nrf_dfu_app_continue\r\n");
    leds_anim_pwr_on();
    ...
    do
    {
        ...
        nrf_dfu_utils_showProgress( (float)(length_left) / (float)(length_total) );
    }
    while(length_left > 0);
