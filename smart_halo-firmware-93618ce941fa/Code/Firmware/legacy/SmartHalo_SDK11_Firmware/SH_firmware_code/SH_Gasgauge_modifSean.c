...
static void set_vmode (bool is_power_saving_mode){
	...
	if (is_power_saving_mode){
		GasGauge_set_bit_in_register(REG_MODE,VMODE);

	}
	else{
		GasGauge_clear_bit_in_register(REG_MODE, VMODE);
	}



//	if (is_power_saving_mode){
//		GasGauge_set_bit_in_register(ADDRESS_OF_GAS_GAUGE,REG_MODE, VMODE);
//	}
//	else{
//		GasGauge_clear_bit_in_register(ADDRESS_OF_GAS_GAUGE,REG_MODE, VMODE);
//	}
}
...
static void set_gg_to_run (bool enable_gas_gauge){
	...
	if (enable_gas_gauge){
		GasGauge_set_bit_in_register(REG_MODE, GG_RUN);
	}
	else{
		GasGauge_clear_bit_in_register(REG_MODE, GG_RUN);
	}
}
...
void set_relative_soc_alarm_level(float percentage_alarm_soc_level){
	...
	GasGauge_writeReg(REG_ALARM_SOC, value_of_register);
}
...
void set_low_voltage_threshold(float low_voltage_value_in_v){
	...
	GasGauge_writeReg(REG_ALARM_VOLTAGE, value_of_register);
}

//Reset the alarm interrupt
void reset_alarm_interrupt(){

	GasGauge_clear_bit_in_register(REG_CTRL, ALM_SOC);
	GasGauge_clear_bit_in_register(REG_CTRL, ALM_VOLT);
}
...
static void set_alarm_interrupt (bool enable_interrupt){
	...
	if (enable_interrupt){
		GasGauge_set_bit_in_register( REG_MODE, ALM_ENA);
	}
	else{
		GasGauge_clear_bit_in_register( REG_MODE, ALM_ENA);
	}
...
static uint8_t get_temperature(){
	...
	SH_twi_read_two_registers(ADDRESS_OF_GAS_GAUGE, REG_TEMPERATURE, data, 2);
...
void GasGauge_set_bit_in_register(size_t register_address, uint16_t bit){

	uint8_t data[2];
	SH_twi_read_two_registers(ADDRESS_OF_GAS_GAUGE, register_address, data, 2);
	uint16_t data16;
	data16 = (data[0] << 8) & 0xFF00;
	data16 += data[1];
	data16 |= 1 << bit;
	GasGauge_writeReg(register_address, data16);
}


void GasGauge_clear_bit_in_register(size_t register_address, uint16_t bit){
...
bool GasGauge_check_bit_in_register(size_t register_address, uint16_t bit){
	...
	data16 = (data[0] << 8) & 0xFF00;
	data16 += data[1];
	bit_value = CHECK_BIT(data16, bit);
	return bit_value;
}

void GasGauge_writeReg( size_t reg , unsigned int data)
{
	ret_code_t err_code;
	uint8_t byteH = ((data >> 8) & 0xFF);
	uint8_t byteL = (data & 0xFF);
	uint8_t devaddress = ADDRESS_OF_GAS_GAUGE<<1;
	...
	err_code = SH_twi_write(ADDRESS_OF_GAS_GAUGE, reg, regdata, 3, false);
...
bool is_there_a_low_condition_with_voltage(){
	...
	if (GasGauge_check_bit_in_register(REG_CTRL, ALM_VOLT)){
		return true;
	}
	else {
		return false;
...
bool is_there_a_low_condition_with_soc(){
	...
	if (GasGauge_check_bit_in_register(REG_CTRL, ALM_SOC)){
		return true;
	}
	else {
		return false;
