...
//added by Sean Beitz:
//this corrupts the peer data so that the bootloader can start up in
//serial transport layer mode
//SYSTEM MUST ABSOLUTELY BE RESET AFTER THIS CALL!!!
void clear_peer_data()
{
	//other option is to memset all the values to 0, memset(mystruct, 0, array_length * sizeof(Mystruct))
//	dfu_ble_peer_data_t empty_peer_data;
//	m_peer_data = empty_peer_data;
	//&m_peer_data = NULL;
	m_peer_data_crc = 0;
}
