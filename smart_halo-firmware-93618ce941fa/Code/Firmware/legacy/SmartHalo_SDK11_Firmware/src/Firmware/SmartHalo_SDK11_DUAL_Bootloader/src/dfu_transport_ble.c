...
#define ADVERTISING_LED_PIN_NO               C_R_PIN                                               /**< Is on when device is advertising. */
#define CONNECTED_LED_PIN_NO                 C_B_PIN                                               /**< Is on when device has connected. */
#endif
...
dfu_ble_peer_data_t* get_m_peer_data_handle()
{
	return &m_ble_peer_data;
}
...
uint32_t dfu_transport_update_start(void)
{
    ...
    //check to see if there is peer data (device is bonded and dfu has been initiated from application)

     err_code = dfu_ble_peer_data_get(&m_ble_peer_data);
     if (err_code != NRF_SUCCESS)
      {
     	err_code = serial_dfu_transport_update_start();
     	APP_ERROR_CHECK(err_code);
...
//    }
//    else
    if(!m_ble_peer_data_valid)//this will never happen because of the serial check above
    {
        ble_gap_addr_t addr;
