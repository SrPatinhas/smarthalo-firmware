...
/*
SmartHalo FirmWare Integration Test with ISSI Drivers and OSRAM LEDS
 Material: 
 SmartHalo ISSI Drivers custom test board
 Method:
 I2C
 
 Sebastien Gelinas (2015/10/01)
*/
...
// CHIP enable pour les LED drivers
#define SDB_PIN 2
...
HaloPixel pixels = HaloPixel(NUMPIXELS,HaloPixel::IMAX_4);
//Adafruit_NeoPixel centerPix = HaloPixel(1, 7, NEO_GRB + NEO_KHZ800);
...
void loop() {
  ...
  for(nStep=0;nStep<8;nStep++)
    left(nStep), delay(500);

  turnOff();
  delay(100);

  for(nStep=0;nStep<8;nStep++)
    slightLeft(nStep), delay(500);

  turnOff();
  delay(100);

  for(nStep=0;nStep<8;nStep++)
    hardLeft(nStep), delay(500);
  ...
  for(nStep=0;nStep<8;nStep++)
    right(nStep), delay(100);

  turnOff();
  delay(500);

  for(nStep=0;nStep<8;nStep++)
    slightRight(nStep), delay(100);

  turnOff();
  delay(500);

  for(nStep=0;nStep<8;nStep++)
    hardRight(nStep), delay(100);
  ...
  switch (eighth) {  //Which step we are
    ...
    case (uint8_t) 7 :  //Sixth set of LED green
      for (int i=0;i<24;i++){
        ...
        pixels.showPixel(NUMPIXELS-6);
        delay(10);
        pixels.setPixelColor(NUMPIXELS-5, pixels.Color(0,64+8*i,0));
        pixels.showPixel(NUMPIXELS-5);
        delay(10);
      }
    break;
...
void pairing(int imin, int imax) {
  for(int i=imin;i<imax;i++) {
    ...
    pixels.showPixel(i); 
    pixels.showPixel(i-1);
    pixels.showPixel(i-2);
    pixels.showPixel(i-3);
    pixels.showPixel(i-4);
    pixels.showPixel(i-5);
    delay(30); 
    pixels.clear();
  }
}
