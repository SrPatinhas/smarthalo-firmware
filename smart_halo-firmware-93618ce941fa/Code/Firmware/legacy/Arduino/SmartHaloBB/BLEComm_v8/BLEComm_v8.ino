...
SmartHalo FirmWare Beta 1.2
 ...
 v8>v7 : Some unnecessary delays removed and better use of pixels.show() 
         Update animation added
         Pairing transition removed, pairing animation will always last the same amount of time
*/
      ...
      if (data < 10 && data != typeOfInstruction){    //TODO : Don't forget to reset typeOfInstruction to uint8_t(10)
        switch(data){
          ...
          case (uint8_t) 7 : //Goal completion
            typeOfInstruction = data;
            break;
          case (uint8_t) 8 : //Pairing animation TODO Take the actual time of the BLE connection
            pairingSequence();
            break;
          case (uint8_t) 9 : //Update animation
            typeOfInstruction = data;
            break;
        }
      } else {
...
void updateNeeded(uint8_t completion){
  pixels.setBrightness(brightness);
  
  if (completion == 1) {
    for (int i=0;i<16;i++){
      for (int j=0;j<NUMPIXELS;j++){
        pixels.setPixelColor(j, pixels.Color(16*i,13*i,0));
      }
      pixels.show();
      delay(8);
    }
