...
#define SHUTDOWN_DELAY 8*60*60*1000

//uint8_t auth_sk[32];
//uint8_t auth_pk[64];
//uint8_t auth_pk_published[64];
...
void auth_do_shutdown(void *ctx) {
    bleapp_shutdown();
}
...
/*
uint8_t * auth_getPubKey() {
    return auth_pk_published;
}
*/
...
void auth_getVersions(uint8_t *buf, uint32_t len) {
    printf("auth_getVersions\r\n");
    uint8_t reply[11];
    uint32_t ptr = 0;
    reply[ptr++] = RET_OK;

    memcpy(reply+ptr, auth_fwversion, 4);
    ptr += 4;

    uint32_t blversion = ((nrf_dfu_settings_t*)0x0007F000)->bootloader_version;
    reply[ptr++] = (uint8_t)((blversion >> 24) & 0xff);
    reply[ptr++] = (uint8_t)((blversion >> 16) & 0xff);
    reply[ptr++] = (uint8_t)((blversion >> 8) & 0xff);
    reply[ptr++] = (uint8_t)((blversion) & 0xff);

    //HW
    reply[ptr++] = (uint8_t)platform_getHW();
...
void auth_getPeriphPubKey(uint8_t *buf, uint32_t len) {
    ...
    uint32_t ptr = 0;
    reply[ptr++] = RET_OK;
    //memcpy(reply+ptr, auth_pk_published, 64);
...
void auth_setCentralPubKey(uint8_t *buf, uint32_t len) {
    ...
    //memcpy(auth_central_pk, buf, 64);
    reverse(auth_central_pk, buf, 32);
    reverse(auth_central_pk+32, buf+32, 32);

    uint8_t secret[32];
    //uECC_shared_secret(auth_central_pk, auth_sk, secret, uECC_secp256r1());
    //ecc_p256_shared_secret_compute(auth_sk, auth_central_pk, secret);
...
void auth_evt_cb( ble_evt_t * p_ble_evt ) {
    ...
    {        
        case BLE_GAP_EVT_CONNECTED:
            //
            if(tst_isFactoryMode()) {
                printf("Factory mode!\r\n");
                disp_setPermissionMask(0x02); //Enable test
            } else {
            ...
            sch_unique_cancel(auth_do_shutdown);
            break;
        case BLE_GAP_EVT_DISCONNECTED:
            if(!auth_password_len) {
                sch_unique_oneshot(auth_do_shutdown, SHUTDOWN_DELAY);
            }
            break;
        default:
            break;
