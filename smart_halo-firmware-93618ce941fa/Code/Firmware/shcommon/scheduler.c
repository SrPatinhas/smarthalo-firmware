...
#define SCHED_MAX_EVENT_DATA_SIZE       MAX(APP_TIMER_SCHED_EVT_SIZE, sizeof(void *))
...
void sch_timer_cb(void * ctx) {
	...
	if(entry->active) {
		if(entry->repeat) {
			...
			app_timer_start(timer_id, APP_TIMER_TICKS(entry->ms, APP_TIMER_PRESCALER), entry);
		} else {
			entry->active = false;		
		}
	}
...
int32_t sch_schedule(bool unique, bool repeat, sch_cb_ctx_t cb, uint32_t ms, void *ctx) {
	...
	if(ptr != -1) {
		...
		app_timer_start(timer_id, APP_TIMER_TICKS(ms, APP_TIMER_PRESCALER), &sch_entries[ptr]);
		return ptr;
	} else {
