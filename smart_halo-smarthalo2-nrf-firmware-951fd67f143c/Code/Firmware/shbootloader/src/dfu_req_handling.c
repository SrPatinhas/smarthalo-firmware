...
static void on_dfu_complete(nrf_fstorage_evt_t * evt)
{
...
static void dfu_data_write_handler(nrf_fstorage_evt_t * evt)
{
    ...
    if (p_init->has_is_debug == false || p_init->is_debug == false)
    {
        ...
        switch (p_init->type)
        {
            case DFU_FW_TYPE_APPLICATION:
                ...
                /*if ((p_init->fw_version != 0) && p_init->fw_version < fw_version)
                {
                    return NRF_DFU_RES_CODE_OPERATION_FAILED;
                }*/
                break;
...
static nrf_dfu_res_code_t nrf_dfu_data_req(void * p_context, nrf_dfu_req_t * p_req, nrf_dfu_res_t * p_res)
{
    ...
    switch (p_req->req_type)
    {
        ...
        case NRF_DFU_OBJECT_OP_WRITE:
            ...
            //printf("Progress %d %d\r\n",s_dfu_settings.progress.firmware_image_offset, m_firmware_size_req);
            leds_bl(true, 100*s_dfu_settings.progress.firmware_image_offset/m_firmware_size_req);
