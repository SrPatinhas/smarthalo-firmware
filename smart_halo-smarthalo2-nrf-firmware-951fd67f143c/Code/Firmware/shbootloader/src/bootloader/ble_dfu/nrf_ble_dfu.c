...
#define DEVICE_NAME                          "SH2_BL"                                              /**< Name of device. Will be included in the advertising data. */
...
static uint32_t advertising_init(uint8_t adv_flags)
{
    ...
    manufData[1] = 1; //bit 0: 1=bootloader, 0=app
 ...
 */
uint32_t ble_dfu_advertising_start()
{
    uint32_t err_code;
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            ...
            //nrf_gpio_pin_set(ADVERTISING_LED_PIN_NO);
            leds_bl(true, 0);
            sch_unique_cancel(nrf_dfu_jumptoApp);
        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            err_code = ble_dfu_advertising_start();
            APP_ERROR_CHECK(err_code);

            m_conn_handle = BLE_CONN_HANDLE_INVALID;

            leds_bl(false, 0);
            sch_unique_oneshot(nrf_dfu_jumptoApp, 5000);
