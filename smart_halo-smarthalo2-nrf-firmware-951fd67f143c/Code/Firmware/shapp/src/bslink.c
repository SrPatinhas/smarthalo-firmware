...
void bslink_recv(void *ctx) {
    ...
    printf("%s: Received: %u %s bytes\n    ", __func__, len, (bslink_payload_crypted) ? "crypted" : "plain");
...
void bslink_on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    {        
        ...
        case BLE_GAP_EVT_DISCONNECTED: {
            ...
            printf("%s: Disconnected, reason %d (0x%x).\r\n",
                   __func__,
                   p_gap_evt->params.disconnected.reason,
                   p_gap_evt->params.disconnected.reason);

            bslink_conn_handle = BLE_CONN_HANDLE_INVALID;
            uint8_t disconnection_msg[3];
            disconnection_msg[0] = UART_SYNC_CONNECTION;
            disconnection_msg[1] = BLE_DISCONNECTED;
            disconnection_msg[2] = p_gap_evt->params.disconnected.reason;
            uart_put_tx_msg(UART_MSG_ID_MSG, UART_CMD_SYNC, disconnection_msg, sizeof(disconnection_msg));
            break;
        }
#ifdef BSLINK_INDICATE
