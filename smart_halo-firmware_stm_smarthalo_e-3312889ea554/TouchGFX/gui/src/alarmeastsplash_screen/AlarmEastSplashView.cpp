#include <gui/alarmeastsplash_screen/AlarmEastSplashView.hpp>

AlarmEastSplashView::AlarmEastSplashView()
{

}

void AlarmEastSplashView::setupScreen()
{
    featureCarrosselDots.setColourOfDot(4, colortype(0xE56B29));
    AlarmEastSplashViewBase::setupScreen();
}

void AlarmEastSplashView::tearDownScreen()
{
    AlarmEastSplashViewBase::tearDownScreen();
}

void AlarmEastSplashView::handleGestureEvent(const GestureEvent& evt)
{
	if (evt.getY() > 270) {
	}
}

void AlarmEastSplashView::handleClickEvent(const ClickEvent& evt){
	if(evt.getY() > 270){
