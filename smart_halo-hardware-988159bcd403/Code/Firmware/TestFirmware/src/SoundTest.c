...
#define VOLUME_PCT_DEF  100

#define ADC_LSB 			0.002734375 // 2.8V / 1024 (10 bits)
#define ADC_VBZSENSE_CONV	7.25 		// 87/12
...
static int volumePct 		= VOLUME_PCT_DEF;
static uint8_t PiezoEnablePin;
...
void SoundTest_printHelp()
{
	...
	nrf_delay_ms(50);
	CommandLineInterface_printLine("vol=<value>:\tSets the volume (%)");
	CommandLineInterface_printLine("vol:\t\tReturns the volume (%)");
}
...
bool SoundTest_parseAndExecuteCommand(char * RxBuff, int cnt)
{
	...
	else if(sscanf(RxBuff, "VOL=%d", &value)==1)
	{
		if(value < VOLUME_PCT_MIN || value > VOLUME_PCT_MAX)
			CommandLineInterface_printf("Invalid VOL: must be between %d and %d%%\r\n", VOLUME_PCT_MIN, VOLUME_PCT_MAX);
		else
		{
			SoundTest_setVolume(value);
			CommandLineInterface_printf("VOL=%d%%\r\n", SoundTest_getVolume());
		}
	}
	...
	else if(strncmp(RxBuff, "VOL", 3)==0)
	{
		CommandLineInterface_printf("VOL=%d%%\r\n",SoundTest_getVolume());
	}
...
{
	int newVolume5b = (int)round(((float)percentVolume / 100.0) * 31.0);

	EasyScale_setFeedbackVoltage(newVolume5b);

	int newVolumeRead = EasyScale_getFeedbackVoltage();

	volumePct = (int)round(((float)newVolumeRead / 31.0) * 100.0);
}
