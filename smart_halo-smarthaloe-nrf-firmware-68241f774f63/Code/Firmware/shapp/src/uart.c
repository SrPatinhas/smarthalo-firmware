...
#define UART_TX_RETRY_TIMER_MS 500
#define UART_RX_RETRY_TIMER_MS 1000
...
typedef struct
{
    ...
    uint8_t u8BufferActivePacket[UART_TX_BUF_SIZE];
    uint8_t u8TXBuffer[UART_TX_BUF_SIZE];
...
bool UartManagePacketPending(void)
{
    poHeader_t poHeader = (poHeader_t)(oUART.u8BufferActivePacket);
    // a ack is pending.
...
//#warning todo: adapt with the schedulor.        
        ...
        {
            // max number of retry.
            if (oUART.u8RetryCounter++ < 3)
            {
                oUART.u32RetryTime = 0;
                // Retry...
                UartSendPacket((uart_msg_id)poHeader->u8Type, (uart_cmd_type)poHeader->u8CMD, poHeader->u8SeqID, &oUART.u8BufferActivePacket[sizeof(oHeader_t)],poHeader->u16Length);
            }
            else
            {
                // drop the packet.
...
{
    if (msg_id == UART_MSG_ID_MSG
        && cmd == UART_CMD_SYNC
        && payload[0] == UART_SYNC_CONNECTION
        && payload[1] == BLE_CONNECTED)
    {
        UartSendPacket(msg_id, cmd, oUART.u8SequenceID, payload, length);
        // hacky AF -- this needs to be fixed right
        // Specifically, this delay offers a little bit of time for the
        // STM to wake-up before sending the next one
        // Fixing it "right" means better wake-up protocol
        nrf_delay_ms(50);
    }
...
void UartSendPacket(uart_msg_id msg_id, uart_cmd_type cmd, uint8_t u8Seq, uint8_t *payload, uint32_t length)
{
    ...
    poHeader_t poHeader = (poHeader_t)oUART.u8TXBuffer;
    ...
    if(length > 0)
    {
        ...
        memcpy(&oUART.u8TXBuffer[u8BufferLength], payload, length);
    ...
    UartProcessCS(&oUART.u8TXBuffer[4], (u8BufferLength - 4), &oUART.u8TXBuffer[u8BufferLength]);
    // To test....
    if (true == oUART.bUTSendBadCS)
    {
        oUART.bUTSendBadCS = false;
        oUART.u8TXBuffer[u8BufferLength]++;
    }
    ...
    {
        // Send the packet
        app_uart_put(oUART.u8TXBuffer[i]);
    }
    // If the packet is not a ack, copy the buffer if the retry is required.
    if (msg_id != UART_MSG_ID_ACK)
    {
        // Cpy the buffer in case of retry.
        memcpy(oUART.u8BufferActivePacket, oUART.u8TXBuffer, u8BufferLength);
        // keep the seqID to ack.
        oUART.u8SeqIDAckToReceive = u8Seq;
